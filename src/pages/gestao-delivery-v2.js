...
export function renderDeliveryGestao(host, initial, changed, save) {
  ...
  host.innerHTML = `<form class="g-form g-delivery"><h2>Entrega em domicílio</h2><label class="g-check"><input name="deliveryAtivo" type="checkbox" ${config.ativo ? 'checked' : ''}>Receber pedidos de delivery</label><label>Pedido mínimo (R$)<input name="minimo" type="text" inputmode="numeric" required data-centavos="${draft.minimoCentavos ?? config.pedidoMinimoCentavos ?? 0}"></label><section><h3>Regiões de entrega</h3><p>Defina a faixa de CEP, a taxa e o prazo de cada região.</p><div class="g-regions"></div><button type="button" class="btn-ghost" id="g-add-region">Adicionar região</button></section><section class="g-schedule"></section><button class="btn-primary" type="submit">Salvar delivery</button><p class="g-result" role="status"></p></form>`;
  ...
  mascaraMoeda(inputMinimo, centavos => { draft.minimoCentavos = centavos; capture(); });

  const capture = () => {
    config.ativo = form.elements.deliveryAtivo.checked;
    draft.minimoCentavos = Number(inputMinimo.dataset.centavos || 0);
    config.regioes = [...regions.children].map(row => ({ id: row.dataset.id, nome: row.querySelector('[name=regiaoNome]').value, cepInicial: row.querySelector('[name=cepInicial]').value, cepFinal: row.querySelector('[name=cepFinal]').value, taxaCentavos: Number(row.querySelector('[name=taxa]').dataset.centavos || 0), prazo: row.querySelector('[name=prazo]').value }));
  ...
  form.onsubmit = async event => {
    ...
    const delivery = { ...config, pedidoMinimoCentavos: draft.minimoCentavos ?? 0, regioes: config.regioes.map(r => ({ id: r.id, nome: r.nome.trim(), cepInicial: r.cepInicial.replace('-', ''), cepFinal: r.cepFinal.replace('-', ''), taxaCentavos: Number(r.taxaCentavos || 0), prazoMinutos: Number(r.prazo) })) };
